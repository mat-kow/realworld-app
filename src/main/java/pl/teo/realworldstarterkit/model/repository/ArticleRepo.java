...
public interface ArticleRepo extends JpaRepository<Article, Long> {
    ...
    @Query(
            value = "SELECT * FROM articles WHERE id IN " +
                    ...
                    "ON tags.id = articles_tags.tag_id WHERE tags.name = ?1) ORDER BY created_at DESC LIMIT ?3 OFFSET ?2",
            nativeQuery = true
    )
    ...
    @Query(
            value = "SELECT * FROM articles ORDER BY created_at DESC LIMIT ?2 OFFSET ?1",
            nativeQuery = true
    )
